...
class RenameFolders(FlaskForm):
    ...
    newFolderName = StringField('Nowa nazwa folderu', validators=[DataRequired()], render_kw={'placeholder': 'Nazwa folderu'})
...
class RenameFiles(FlaskForm):
    """formularz zmiany nazwy pliku"""
    newFileName = StringField('Nowa nazwa pliku', validators=[DataRequired()], render_kw={'placeholder': 'Nazwa pliku'})
    submit = SubmitField('Zmień nazwę')
...
def dashboard():
    ...
    if request.args.get('path'):
        ...
        fullPath = os.path.join(app.config['UPLOAD_PATH'], path)
        onlyFiles = [file for file in os.listdir(fullPath) if os.path.isfile(os.path.join(fullPath, file))]
        onlyFolders = [folder for folder in os.listdir(fullPath) if os.path.isdir(os.path.join(fullPath, folder))]

        folders = Folders.query.filter(Folders.folderName.in_(onlyFolders)).all()
        files = Files.query.filter(Files.fileName.in_(onlyFiles)).all()
    else:
        onlyFiles = [file for file in os.listdir(app.config['UPLOAD_PATH']) if os.path.isfile(os.path.join(app.config['UPLOAD_PATH'], file))]
        onlyFolders = [folder for folder in os.listdir(app.config['UPLOAD_PATH']) if os.path.isdir(os.path.join(app.config['UPLOAD_PATH'], folder))]
    ...
    return render_template('dashboard.html', title='Dashboard', users=users, addUser=addUser, editUser=editUser, editUserPass=editUserPass, search=search, createFolder=createFolder, uploadFile=uploadFile, folders=folders, files=files, renameFolder=renameFolder, renameFile=renameFile, path=path)
...
def renameFolder():
    ...
    if newFolderName != '':
        path = False
        if request.form.get("path", False):
            path = request.form['path']
        id = request.form['id']
        folder = Folders.query.filter_by(id=id).one()
        if path:
            os.rename(os.path.join(app.config['UPLOAD_PATH'], path, folder.folderName), os.path.join(app.config['UPLOAD_PATH'], path, newFolderName))
        else:
            os.rename(os.path.join(app.config['UPLOAD_PATH'], folder.folderName), os.path.join(app.config['UPLOAD_PATH'], newFolderName))
        folder.folderName = newFolderName
        db.session.commit()
        flash('Zmiana nazwy folderu pomyślna', 'success')
        return redirect(url_for('dashboard'))
...
def deleteFolder():
    if request.method == 'GET':
        ...
        db.session.commit()
        if path:
            os.rmdir(os.path.join(app.config['UPLOAD_PATH'], path, folder.folderName))
        else:
...
def renameFile():
    ...
    if newFileName != '':
        ...
        withExt = newFileName.split(".")
        if withExt[-1] not in uploadExt:
            newFileName = newFileName + "." + file.type

        if path:
            os.rename(os.path.join(app.config['UPLOAD_PATH'], path, file.fileName), os.path.join(app.config['UPLOAD_PATH'], path, newFileName))
        else:
            os.rename(os.path.join(app.config['UPLOAD_PATH'], file.fileName), os.path.join(app.config['UPLOAD_PATH'], newFileName))

        file.fileName = newFileName
        db.session.commit()
        flash('Zmiana nazwy pliku pomyślna', 'success')
        return redirect(url_for('dashboard'))
...
def deleteFile():
    if request.method == 'GET':
        ...
        if path:
            os.remove(os.path.join(app.config['UPLOAD_PATH'], path, file.fileName))
        else:
            os.remove(os.path.join(app.config['UPLOAD_PATH'], file.fileName))
        flash('Plik usunięty poprawnie', 'success')
        return redirect(url_for('dashboard'))
